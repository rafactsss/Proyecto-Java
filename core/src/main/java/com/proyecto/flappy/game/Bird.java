...
public class Bird extends Entity implements Collidable {
    ...
    public Bird() {
        ...
        hitboxWidth  = config.BIRD_WIDTH  * 0.55f;
        hitboxHeight = config.BIRD_HEIGHT * 0.45f;
    ...
    public Bird(String spritePath) { this(); }
    ...
    public void render(SpriteBatch batch) {
        GameConfig config = GameConfig.getInstance();
        batch.draw(region, position.x, position.y, config.BIRD_WIDTH, config.BIRD_HEIGHT);
    }

    public void jump() { velocity.y = 250f; }

    @Override
    public Rectangle getHitbox() { return bounds; }
